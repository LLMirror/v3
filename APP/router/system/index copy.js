...
function startCheckDateJob() {
  console.log("定时任务启动中：每天凌晨 1:30 自动检查并发起审批（提前一周）");

  // 每天凌晨 1:30 执行（当前配置为 1:51，可按需调整为 '30 1 * * *'）
  schedule.scheduleJob('45 22 * * *', async () => {
    try {
      const today = dayjs().startOf('day');
      const monthStart = today.startOf('month').format('YYYY-MM-DD');
      const monthEndPlus7 = today.endOf('month').add(7, 'day').format('YYYY-MM-DD');

      // 1) 拉取当月及后面一周的待款数据
      const sql = `SELECT 收款账户名, 收款支行, 收款账户, 付款金额, 付款日期, 备注 FROM pt_cw_dk WHERE 付款日期 BETWEEN ? AND ?`;
      const { result: dkRows = [] } = await pools({ sql, val: [monthStart, monthEndPlus7], res: {}, req: null });
      if (!dkRows || dkRows.length === 0) {
        console.log(`[定时任务] ${today.format('YYYY-MM-DD')} 当月及后面一周无待款数据`);
        return;
      }

      // 2) 找到“今天在付款日前7天内(1~7天)”的记录（包含13号属于15号的提前一周内）
      const targetRows = dkRows.filter(row => {
        const payDate = dayjs(row.付款日期);
        if (!payDate.isValid()) return false;
        const diffDays = payDate.diff(today, 'day'); // 今天距离付款日还剩几天
        const subtractDate = payDate.subtract(7, 'day');
        console.log('[定时任务] 检查付款日期：', {
          付款日期: payDate.format('YYYY-MM-DD'),
          今天: today.format('YYYY-MM-DD'),
          '付款日期-7天': subtractDate.format('YYYY-MM-DD'),
          距离付款日天数: diffDays,
          '是否在提前一周内(1-7天)': diffDays > 0 && diffDays <= 7
        });
        // 提前一周内（今天在付款日前，且剩余天数 1~7 天）
        return diffDays > 0 && diffDays <= 7;
      });

      if (targetRows.length === 0) {
        console.log(`[定时任务] 今日非任何付款的提前一周，跳过。`);
        return;
      }

      // 3) 准备发起审批的上下文（选择一个具备 employeeUserId/departmentId 的用户）
      const userSql = `SELECT id, ddtk, more_id AS moreId, employee_user_id AS employeeUserId, department_id AS departmentId FROM user 
                       WHERE employee_user_id IS NOT NULL AND department_id IS NOT NULL 
                       ORDER BY id ASC LIMIT 1`;
      const { result: userRows = [] } = await pools({ sql: userSql, res: {}, req: null });
      if (!userRows.length) {
        console.warn(`[定时任务] 未找到具备钉钉发起审批所需信息的用户（employeeUserId/departmentId）。`);
        return;
      }
      const user = userRows[0];

      const moreSql = `SELECT id, name, remark, app_key AS appKey, app_secret AS appSecret FROM more WHERE id = ? LIMIT 1`;
      const { result: moreRows = [] } = await pools({ sql: moreSql, val: [user.moreId], res: {}, req: null });
      if (!moreRows.length) {
        console.warn(`[定时任务] 用户 moreId 未配置到 more 表。`);
        return;
      }
      const more = moreRows[0];
      const appKey = more.appKey;
      const appSecret = more.appSecret;
      const remark = more.remark; // 流程名称
      if (!appKey || !appSecret || !remark) {
        console.warn(`[定时任务] more 表缺少 appKey/appSecret/remark 配置。`);
        return;
      }
      ...
      const refreshToken = async () => {
        ...
        if (dtRes.data?.errcode === 0) {
          token = dtRes.data.access_token;
          const updateSql = `UPDATE user SET ddtk = ?, update_time = ? WHERE id = ?`;
          await pools({ sql: updateSql, val: [token, moment().format('YYYY-MM-DD HH:mm:ss'), user.id], res: {}, req: null });
          console.log('[定时任务] 已刷新钉钉 token');
        } else {
          throw new Error(`钉钉获取token失败：${dtRes?.data?.errmsg || '未知错误'}`);
        }
      };

      const getProcessCodeByName = async (tk, name) => {
        try {
      ...
      // 若无 token 或 token 失效则刷新
      if (!token) {
        await refreshToken();
      } else {
        const check = await getProcessCodeByName(token, remark);
        if (!check.ok) {
      ...
      // 使用最新 token 查询流程编码
      const codeRes = await getProcessCodeByName(token, remark);
      if (!codeRes.ok || !codeRes.data?.result?.processCode) {
        console.warn(`[定时任务] 获取流程编码失败：`, codeRes.error || codeRes.data);
        return;
      }
      const processCode = codeRes.data.result.processCode;
      ...
      for (const row of targetRows) {
        const formComponentValues = [
          { name: '付款内容', value: String(row.备注 || '') },
          { name: '金额（元）', value: String(row.付款金额 || '') },
          { name: '收款账户名', value: String(row.收款账户名 || '') },
          { name: '开户行', value: String(row.收款支行 || '') },
          { name: '收款账户', value: String(row.收款账户 || '') }
        ];
        ...
        while (attempt < 3 && !success) { // 第一次 + 2次重试
          attempt++;
          try {
            ...
            console.log(`[定时任务] 发起审批成功：instanceId=${dtRes.data?.result?.instanceId || dtRes.data?.instanceId || '未知'}`);
            success = true;
          } catch (error) {
            lastErrMsg = error.response?.data?.message || error.response?.data?.errmsg || error.message || '未知错误';
            console.warn(`[定时任务] 发起审批失败（第${attempt}次）：${lastErrMsg}`);
            // 间隔 1 秒再试
        ...
        if (!success) {
          console.error(`[定时任务] 最终失败：无法为付款日期 ${row.付款日期} 发起审批。原因：${lastErrMsg}`);
        }
      }

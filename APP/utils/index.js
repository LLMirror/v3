...
import {errLog} from './err.js'
import pools from './pools.js';

//日志记录
export default {
    setToken({uid,captcha,name="user"}) {
        return jwt.sign({uid, captcha}, name, {
    ...
    async existName({sql, name,msg="名称已存在！",req,res}) {
        if(!name) return true;
        let {result}=await pools({sql,res,req,val:[name]});
        if (result.length > 0){
    ...
    async judgeUserName({sql,sqlName="name", name,id,req,res}) {
        let {result}=await pools({sql,val:[id],res,req});
    ...
    async getUserRole(req, res) {
        ...
        let {result}=await pools({sql:userSql,val:[user.rolesId],res,req});
        if (result.length == 0) {
    ...
    async checkPermi({req,res,role=[],admin=false,run=false}){
        ...
        let {result}=await pools({sql,val:[userRole.userRole],res,req});
        try {
    ...
    async getSum({sql="",name,res,req}) {
        ...
        let {result:resultRes}=await pools({sql:sqlRes,res,req});
     ...
     * @param val 值
     * */
    setAssign(sql,name="",val=""){
        if(this.exist(val)) sql+=` AND ${name}=${val}`;
        return sql
    },
    /**
     * 判断是否为空
     ...
     * @param req any
     * */
     async getFileMenu (req){
        const {menuId,id}=req.body;
        const imgMenuId=menuId||id;
        if(!this.exist(imgMenuId)) return false;
        let sql = `SELECT name FROM file_menu WHERE id=?`;
        const menuRes=await pools({sql,val:[imgMenuId],res:{},req});
        if(!menuRes||menuRes.result.length<=0) return false;
        return menuRes.result[0].name;
    },
    /**
     * 删除超过30天的日志记录
     * */
    deleteLogs(){
         const sql='DELETE FROM logs WHERE create_time < DATE_SUB(NOW(), INTERVAL 30 DAY) ORDER BY create_time ASC LIMIT 100';
         pools({sql});
    }
};

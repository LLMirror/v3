<template>
    <div class="container">
        ...
        <!-- 列表表格 -->
        <el-table :data="uploadLists" border @selection-change="handleSelectionChange"
        ...
        <div v-if="columns.length">
            <el-table :data="pagedData" border height="500">
...
import { importData } from "@/api/system/index.js";
...
async function pushDataToDB(row) {
    if (!row.data?.length || !row.dbName) return;

    console.log(`开始导入 ${row.dbName} 数据...`);
    const progressItem = currentFileProgress.value[0];
    progressItem.stage = '上传中';
    progressItem.progress = 0;

    const BATCH_SIZE = 1000; // 分批上传
    ...
    try {
        for (let i = 0; i < total; i += BATCH_SIZE) {
            console.log(`正在上传 ${row.name} 数据...`);
            const batch = row.data.slice(i, i + BATCH_SIZE);
            ...
            if (result.code !== 200) {
                progressItem.stage = '上传失败';
                ElMessage.error(`${row.name} 数据导入失败: ${result.msg}`);
                return;
            }

            sent += batch.length;
            progressItem.progress = Math.min(100, Math.floor((sent / total) * 100));
        }

        progressItem.stage = '完成';
        progressItem.progress = 100;
        ElMessage.success(`${row.name} 数据导入完成`);

        // 清理缓存
        row.data = [];
        row.files = [];
        await deleteFileData(row.name);
        saveListCache();

    } catch (err) {
        console.error(err);
        progressItem.stage = '上传失败';
        ElMessage.error(`${row.name} 数据导入失败: ${err.message}`);
    }
}
...
async function readFiles(row) {
    ...
    currentFileProgress.value = row.files.map(f => ({
        name: f.name,
        type: f.name.split('.').pop(),
        progress: 0,       // 当前进度 0~100
        stage: '读取中'     // 可显示 "读取中" / "上传中" / "完成"
    }));
...
// 停止读取
function stopReading() { stopFlag = true; tableData.value = []; columns.value = []; progressDialogVisible.value = false; }
